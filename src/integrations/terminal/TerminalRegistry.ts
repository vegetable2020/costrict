...
export class TerminalRegistry {
	...
	private static destroyTerminal(terminal: RooTerminal) {
		console.info(`[TerminalRegistry] Destroying terminal ${terminal.id} (created at ${new Date((terminal as any).createdAt).toISOString()})`)
	...
	private static checkAndCleanupNotBusyTerminals() {
		...
		console.info(`[TerminalRegistry] Non-busy terminals count (${notBusyTerminals.length}) exceeds limit (${this.maxNotBusyTerminals})`)
		...
		if (remainingNotBusyTerminals.length > this.maxNotBusyTerminals) {
			console.warn(`[TerminalRegistry] Still have ${remainingNotBusyTerminals.length} non-busy terminals after cleanup (some have unretrieved output)`)
		}
	}
}
